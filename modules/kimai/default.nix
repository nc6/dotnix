...
in
{
  ...
  config = mkIf (eachSite != { }) (mkMerge [
...

    {
      ...
      environment.systemPackages = mapAttrsToList (hostName: cfg:
        pkgs.writeShellScriptBin "kimai-console-${hostName}" ''
          exec sudo -u ${user} ${pkg hostName cfg}/bin/console "$@"
        ''
      ) eachSite;
    }
    ...
    (mkIf (cfg.webserver == "caddy") {
      services.caddy.virtualHosts = mapAttrs (hostName: cfg: {
        extraConfig = ''
          root * ${pkg hostName cfg}/share/php/kimai/public
          php_fastcgi unix/${config.services.phpfpm.pools."kimai-${hostName}".socket}
          file_server
        '';
      }) eachSite;
    })
